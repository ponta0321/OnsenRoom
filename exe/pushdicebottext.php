...
if(strpos($game_key,'bac_')!==false){
	...
	if(isset($bac_gamelist[$game_key]) && $bac_game_dicebot!='DiceBot'){
		...
		$result_dhm=classBCDiceAPIv2::info('https://bcdice.trpgonsen.click/',$bac_game_dicebot);
		// 出力された説明文を表示する
		if(isset($result_dhm['help_message'])){
			$result.=$result_dhm['help_message'];
		// 出力されたエラーを表示する
		}else{
			$result.='<span style="color:#DDD;">ダイスボット説明の取得時にエラーが発生しました。'.(isset($result_dhm['reason'])?'｛'.$result_dhm['reason'].'｝':'').'</span>';
		}
	}
// オンセンdb
}else{
