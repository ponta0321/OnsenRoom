<?php
    ...
    $json_data=getCharListFromLobbyServer($principal_id,$principal_ip);
                ...
                if($room_chara_column->char_owner==$principal_id){
                    foreach($characterlist_array as $load_chara_column){
                        if($room_chara_column->char_id==$load_chara_column[0]){
                            if($c_character->saveDataFromLobby($room_id,
                                                               getCharDataFromLobbyServer($room_chara_column->char_id,
                                                                                          $principal_id,
                                                                                          $principal_ip))){
                                // キャラクター画像データの読み込み
                                $image_list=array();
