...
/* common */
  ...
  define('APP_VERSION','1.04.08'); // バージョン
/* password */
...
/* ダイスボット（BCDice専用） */
  define('BAC_ENDPOINT','https://bcdice.trpgonsenroom.com'); // BCDiceAPIのエンドポイント
/* ダイスボット（オンセンdb専用） */
...
// 基本的には変更しない、ロビーサーバを自設する場合のみ変更
  ...
  define('LOBBY_DOMAIN','trpgsession.com'); // ロビーサイトのドメイン デフォルト trpgsession.com
/* 以下、変更禁止 */
  ...
  define('LOBBY_URL_ROOT',LOBBY_TRANSFER_PROTOCOL.'//'.LOBBY_DOMAIN.'/'); // ルームに入るための入り口となるサイトのURL（https:なども含む）を入力する 変更しない場合は https://trpgsession.com/
