...
header('Last-Modified: '.gmdate( 'D, d M Y H:i:s' ).' GMT');
    ...
    if(file_exists(DIR_ROOT.'s/list/trpg_sys_list.php')){
        @include(DIR_ROOT.'s/list/trpg_sys_list.php');
    }
    // キャラシート・テンプレートの読み込み
    $template_chara=array();
    if(file_exists(DIR_ROOT.'s/list/templatechar.php')){
        @include(DIR_ROOT.'s/list/templatechar.php');
    }
    if(0<count($global_trpg_name)){
        foreach($global_trpg_name as $key => $value){
            if(!isset($template_chara[$key])){
    ...
    if($observer_flag==0){
        ...
        getTrpgSystemInfoFromLobbyServer($last_update_global_trpg_name,$global_trpg_name,$last_update_template_chara,$template_chara);
        // 持ちキャラのロード
