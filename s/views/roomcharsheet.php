...
<script>
    function putValueForSelect(){
        var templateChara={};
        <?php
            foreach($template_chara as $key => $value){
                echo 'templateChara["'.$key.'"]={0:"'.addcslashes(str_replace(array("\r\n","\n","\r"),'\n',$value[0]),'"').'",1:"'.addcslashes(str_replace(array("\r\n","\n","\r"),'\n',$value[1]),'"').'",2:"'.addcslashes(str_replace(array("\r\n","\n","\r"),'\n',$value[2]),'"').'",3:"'.addcslashes(str_replace(array("\r\n","\n","\r"),'\n',$value[3]),'"').'"};';
            }
        ?>
        var key=document.getElementById('id_cs_s_type').value;
        document.getElementById('id_cs_t_type').value=templateChara[key][0];
        document.getElementById('id_cs_t_da').value=templateChara[key][1];
        document.getElementById('id_cs_t_db').value=templateChara[key][2];
        document.getElementById('id_cs_t_dc').value=templateChara[key][3];
    }
